...
void move(struct dock*, int, int, int);
...
void move(struct dock *d, int dst, int src, int amnt) { 

	char *dstptr, *srcptr;

	dstptr = d->stacks[dst] + d->ptrs[dst];

	srcptr = (d->stacks[src] + d->ptrs[src]) - amnt;

	memcpy(dstptr, srcptr, amnt);
